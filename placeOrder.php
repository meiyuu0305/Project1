...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $stmt = $con->prepare("INSERT INTO orders (name, phone, creditCard, zip, shipping_loca, order_id) VALUES (?,?,?,?,?,?)");
...
if (!$stmt) {
    ...
    $stmt -> bind_param("sssiss", $name, $phone, $creditCard, $zip, $address, $order_id);
    $str_product = implode(" ",$product); 
    $stmt1 -> bind_param("isi", $order_id, $str_product, $total);
